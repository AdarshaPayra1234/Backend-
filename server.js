...
app.post('/api/login', async (req, res) => {
  ...
  const lowercaseEmail = email.toLowerCase(); // Ensuring email comparison is case-insensitive
  ...
  try {
    ...
    if (!isMatch) {
      console.log(`Password mismatch for email: ${lowercaseEmail}`);
      return res.status(400).json({ message: 'Invalid credentials' });
    }

    // If credentials are correct, generate JWT token
    const token = generateJWT(user);
    console.log(`Login successful for email: ${lowercaseEmail}`);
...
app.post('/api/update-password', async (req, res) => {
  ...
  try {
    ...
    if (user.otp !== otp || user.otpExpiration < Date.now()) {
      console.log(`OTP mismatch or expired for email: ${email}`);
